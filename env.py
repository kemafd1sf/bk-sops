...
# 异步重试 node_callback 的间隔时间（秒）
ASYNC_NODE_CALLBACK_RETRY_INTERVAL = int(
    os.getenv("ASYNC_NODE_CALLBACK_RETRY_INTERVAL", os.getenv("BKAPP_ASYNC_NODE_CALLBACK_RETRY_INTERVAL", 20))
)
...
BK_DATA_REPORT_API_URL = os.getenv("BK_DATA_REPORT_API_URL", "")

# webhook配置
MAX_WEBHOOK_RETRY_TIMES = int(os.getenv("MAX_WEBHOOK_RETRY_TIMES", 5))
MAX_WEBHOOK_RETRY_INTERVAL = int(os.getenv("MAX_WEBHOOK_RETRY_INTERVAL", 600))
...
APIGW_MCP_SERVER_ID_HEADER = os.getenv("APIGW_MCP_SERVER_ID_HEADER", "HTTP_X_BKAPI_MCP_SERVER_ID")

# 监控策略分页大小
MONITOR_PAGE_SIZE = int(os.getenv("MONITOR_PAGE_SIZE", 100))

# 标准运维智能体API地址
BK_SOPS_AGENT_HOST = os.getenv("BK_SOPS_AGENT_HOST")

# APIGW环境
BKAPP_APIGW_ENVIRONMENT = os.getenv("BKAPP_APIGW_ENVIRONMENT")

# 标准运维智能体版本
AGENT_VERSION = os.getenv("AGENT_VERSION", "1.0.0")

# 是否开启AI分析通知
ENABLE_AI_NOTIFICATION = int(os.getenv("ENABLE_AI_NOTIFICATION", 0)) == 1

# AI失败任务分析通知标题
AI_ANALYSIS_FAIL_TASK_NOTIFY_TITLE = os.getenv("AI_ANALYSIS_FAIL_TASK_NOTIFY_TITLE", "标准运维AI错误分析报告")

# AI总结报告通知标题
AI_SUMMARY_TASK_NOTIFY_TITLE = os.getenv("AI_SUMMARY_TASK_NOTIFY_TITLE", "标准运维任务执行情况AI总结报告")
